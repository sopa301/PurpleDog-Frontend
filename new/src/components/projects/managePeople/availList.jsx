...
export default function AvailList(props) {
  ...
  function mapTasks(avail) {
    ...
    async function handleDelete() {
      await axios.delete(import.meta.env.VITE_API_URL + '/avail', {
    ...
    return <ListItem key={avail.id}>
        <Card paddingY="5px" paddingX="5px">
          <Flex>
            <Container>{avail.toString()}</Container>
            <Spacer/>
            <Button onClick={handleEdit}>Edit</Button> 
            <CButton content="Delete" onClick={handleDelete} children={{colorScheme:"red"}}/> 
          </Flex>
        </Card>
      </ListItem>;
  }
  ...
  async function addAvailOnSubmit(values, actions) {
    ...
    await axios.put(import.meta.env.VITE_API_URL + '/avail', {
    ...
    .then(function (response) {
      ...
      onClose();
      props.setArray(x => [...x, new Availability(response.data.interval_id, interval)]);
    })
    .catch(function (error) {
      toast({
  ...
  async function editAvailOnSubmit (values, actions) {
    const interval = Interval.fromDateTimes(values.start, values.end);
    await axios.patch(import.meta.env.VITE_API_URL + '/avail', {
      user_id: props.person.id,
      interval: interval.toISO({suppressSeconds: true}),
    })
    .then(function (response) {
      ...
      onClose();
      const index = props.array.findIndex((avail) => avail.id === activeAvail.current.id);
