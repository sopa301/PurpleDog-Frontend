          ...
          >
            {formik => (
              <Form>
                <Field name='name'>
                  {({ field, form }) => (
                    <FormControl isInvalid={formik.errors.name && form.touched.name}>
                ...
                <Field name='pax'>
                  {({ field, form }) => (
                    <FormControl isInvalid={formik.errors.pax && form.touched.pax}>
                      ...
                      <Input 
                        ...
                        onChange={
                          (v) => {
                            ...
                            formik.setFieldValue('assignees', isInDesiredForm(v.target.value) 
                              ? createAssigneeArray(v.target.value, formik.values.assignees)
                              : [])
                          }
                        } 
                      />
                            ...
                            <Select 
                              ...
                              onChange={(v) => 
                                formik.setFieldValue('assignees', 
                                  [...formik.values.assignees.slice(0, index), v.target.value,
                                    ...formik.values.assignees.slice(index + 1, formik.values.assignees.length)])}>
                ...
                <Field name='priority'>
                  {({ field, form }) => (
                    <FormControl>
                      ...
                      <RadioGroup 
                          onChange={(val) => formik.setFieldValue("priority", val, false)} 
                  ...
                  {({ field, form }) => (
                    <FormControl isInvalid={formik.errors.interval}>
                      ...
                      <DateTimeField 
                        ...
                        onChange={(val) => {formik.setFieldValue("start", val, true)}} 
                        value={formik.values.start} 
                        label='Start Time'/>
                      <div><br/></div>
                      <DateTimeField 
                        format="dd/MM/yyyy hh:mm a" 
                        onChange={(val) => {formik.setFieldValue("end", val, true)}} 
