...
export default function TaskSettings(props) {
    ...
    function mapTG(task) {
        ...
        if (task.user_id > 0) {
            ...
            const name = props.proj.people.filter(x => Number(x.id) === Number(task.user_id))[0].name;
    ...
    async function handleEdit(values, actions) {
        ...
        for (let i = 0; i < values.assignees.length; i++) {
            array[i] = new Task(null,
                interval, values.assignees[i], "idk", props.proj.id, values.priority, props.taskGroup.id);
        }
        axios.patch(import.meta.env.VITE_API_URL + '/taskgroup', {
            ...
            const newTaskGroup = new TaskGroup(props.taskGroup.id, values.name, array, values.pax, values.priority);
            const newTaskGroups = [...props.proj.taskGroups.slice(0, props.index),
                newTaskGroup,
                ...props.proj.taskGroups.slice(props.index + 1, props.proj.taskGroups.length)];
    ...
    async function handleDelete() {
        await axios.delete(import.meta.env.VITE_API_URL + '/taskgroup', {
            group_id: props.taskGroup.id
        }).then(function (response) {
            toast({
            ...
            const newTaskGroup = [...props.proj.taskGroups.slice(0, props.index),
                ...props.proj.taskGroups.slice(props.index + 1, props.proj.taskGroups.length)];
    ...
    return <AccordionItem>
        <AccordionButton padding="5px">
            <Flex w="100%">
                <Box textAlign="left" fontWeight='semibold'>{props.taskGroup.name}</Box>
                <Spacer />
                <AccordionIcon/>
            </Flex>
        </AccordionButton>
        ...
        <TaskMenu
            ...
            title="Edit Task"
            proj= {props.proj}
            initialValues={{name: props.taskGroup.name, 
