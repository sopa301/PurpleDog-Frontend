    ...
    async function handleSubmit(values, actions){
        ...
        for (let i = 0; i < values.assignees.length; i++) {
            array[i] = new Task(null,
                interval, values.assignees[i], "idk", props.proj.id, values.priority, null);
        }
        await axios.put(import.meta.env.VITE_API_URL + '/taskgroup', {
            ...
            const newTaskGroup = new TaskGroup(response.data.id, values.name, array, values.pax, values.priority);
            ...
            const newTaskGroup = new TaskGroup(1, values.name, array, values.pax, values.priority);
            const newTaskGroups = [...props.proj.taskGroups, newTaskGroup];
            props.update(newTaskGroups);
            onClose();
        });
